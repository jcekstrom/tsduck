...
TSDuck provides native https://nixos.org[Nix] flakes for Linux, macOS, and BSD systems.
...
[source,shell]
----
$ nix run ./pkg/nix#default -- --version
$ nix run ./pkg/nix#tsp -- --help
$ nix run ./pkg/nix#tsanalyze -- input.ts
----
...
`?dir=` parameter to install an exact version:
